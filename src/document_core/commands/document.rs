...
impl DocumentCore {
    pub fn from_bytes(data: &[u8]) -> Result<DocumentCore, HwpError> {
        ...
        let hwpx_package_snapshot = if source_format == DocumentSourceFormat::Hwpx {
            match crate::parser::hwpx::reader::HwpxPackageSnapshot::from_bytes(data) {
                ...
                Err(error) => {
                    eprintln!(
                        "warning: failed to capture HWPX package snapshot: {}",
                        error
                    );
                    None
                }
    ...
    pub fn export_docx_native(&self) -> Result<Vec<u8>, HwpError> {
        crate::export::docx::export_document(self)
    }
