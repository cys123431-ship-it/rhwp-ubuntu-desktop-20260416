...
이 프로젝트는 웹 기반의 `rhwp` 엔진을 활용해, 우분투와 윈도우 환경에서 파일 더블 클릭만으로 문서를 바로 열고 읽고 편집할 수 있는 네이티브 경험을 제공하는 것을 목표로 제작되었습니다. 비영리 오픈소스 프로젝트이며, 현재 이 저장소에서 직접 검증된 설치 패키지는 Ubuntu용 `.deb` 파일입니다.
...
현재 릴리즈에서 제공하는 설치 파일은 Ubuntu용 `.deb` 패키지입니다.
...
- MIME 연결 및 더블 클릭 대상 등록: 반영
- Windows 데스크톱 지원: 목표 범위에 포함되지만, 이 저장소의 현재 릴리즈 문서에서는 Ubuntu 설치 패키지를 기준으로 안내합니다.
