...
사용자에게 표시되는 제품명은 `Geulbit X`입니다. 기존 설치본과의 업그레이드, 파일 연결, 사용자 데이터 경로 호환성을 유지하기 위해 Linux 실행 파일명 `rhwp`, 데스크톱 등록 파일 `rhwp.desktop`, Tauri 식별자 `io.rhwp.desktop`, Rust crate/엔진 이름 `rhwp`는 내부 식별자로 유지합니다.
